...
class SettingsController extends MainAdminController
{
    ...
    public function web_ads_settings()
    {
        ...
        $allAds = WebAds::paginate(10);

        return view('admin.pages.settings.web_ads',compact('page_title','allAds'));
    }
   ...
   public function update_web_ads_settings(Request $request)
{
    ...
    $validated = $request->validate([
        'home_top_text' => 'required', // Image URL
        'ad_url' => 'required|url',    // Ad URL
    ]);

    try {
        // Create a new WebAds record
        $web_ads_obj = new WebAds();
        $web_ads_obj->home_top = addslashes($validated['home_top_text']);
        $web_ads_obj->ad_url = addslashes($validated['ad_url']);

        // Generate the HTML code for the banner ad
        $html_code = '
        <a href="' . htmlspecialchars(url($validated['ad_url'])) . '" target="_blank">
            <img src="' . htmlspecialchars($validated['home_top_text']) . '" alt="Banner Ad" style="width:728px; height:90px; display:block;">
        </a>
    ';
        ...
        // Save the HTML code in a field, e.g., `home_bottom` or any appropriate field
        $web_ads_obj->home_bottom = addslashes($html_code);

        // Save the new record
        $web_ads_obj->save();

        // Flash success message to the session
        Session::flash('flash_message', trans('Successfully Created'));

        // Redirect back to the previous page
        return redirect()->back();
    } catch (\Exception $e) {
        // Flash error message to the session
        Session::flash('error_message', trans($e->getMessage()));

        // Log the error for debugging purposes
        \Log::error('WebAds Update Error: ' . $e->getMessage());
    ...
    public function update_site_maintenance(Request $request)
    {
        ...
        Session::flash('flash_message', trans('Successfully_updated'));
    ...
    public function store_generateScreenshot(Request $request)
    {
        ...
        if (is_resource($process)) {
            ...
            if ($returnVar === 0) {
                ...
                $fullError = $bundledError . 'Error generating screenshot. ' . $pathMsg . '. Exit Code: ' . $returnVar . '. Error Output: ' . $errorOutput . '. Stdout: ' . $output;
                return redirect()->back()->with('error', $fullError);
            }
        } else {
    ...
    public function web_ads_settings_delete($id){
        $webAds = WebAds::findOrFail($id);
        $webAds->delete();
        return redirect()->back()->with('flash_message','Banner ad deleted successfully!');
    }
    public function adButtonsBanners($type){



      if ($type == 'buttons') {
        $page_title = 'Buttons';
        $data = ButtonsBanners::where('type', 'buttons')->paginate(10);
      }
      else{
          $page_title = 'Banners';
    ...
    public function createBtnBanner(Request $request)
{
    ...
    $validatedData = $request->validate([
        ...
        'home_top_text' => 'nullable|string|max:255', // Image is optional
    ...
    if ($request->type === 'banners') {
        $validatedData['title'] = ''; // Set title to an empty string for banners
    ...
    $validatedData['title'] = $validatedData['title'] ?? ''; // Default title to empty string
    ...
    public function deleteBtnBanner($id){
        ...
        $btnBanner->delete();
        return redirect()->back()->with('flash_message','Button/Banner deleted successfully!');
