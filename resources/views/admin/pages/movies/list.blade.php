...
@section('content')
                                        ...
                                        <div class="card m-b-20">
                                            <div class="wall-list-item">
                                                ...
                                                @if (isset($movies->video_image_thumb))
                                                    <img class="card-img-top thumb-xs img-fluid"
                                                        src="{{ URL::to('/' . $movies->video_image_thumb) }}"
                                            ...
                                            <div class="card-body p-3">
                                                <h4 class="card-title book_title mb-3 d-flex align-items-center">
                                                    {{ stripslashes($movies->video_title) }}</h4>
                                                <a href="{{ url('admin/movies/edit_movie/' . $movies->id) }}"
                                                    ...
                                                    data-toggle="tooltip" title="{{ trans('words.edit') }}"><i
                                                        class="fa fa-edit" @disabled(true)></i></a>
                                                ...
                                                <a href="javascript:void(0);"
                                                    ...
                                                    <i class="fa fa-play" @disabled(true)></i>
                                                </a>
                                                ...
                                                <a href="#"
                                                    ...
                                                    data-id="{{ $movies->id }}"><i class="fa fa-remove"></i></a>
                                                @if (Auth::user()->usertype == 'Admin')
                                                    <a class="ml-2 fl-right mt-1" href="Javascript:void(0);"
                                                        data-toggle="tooltip"
                                                        title="@if ($movies->status == 1) {{ trans('words.active') }} @else {{ trans('words.inactive') }} @endif"><input
                                                            type="checkbox" name="category_on_off" id="category_on_off"
                                                            value="1" data-plugin="switchery" data-color="#28a745"
                                                            data-size="small" class="enable_disable"
                                                            data-id="{{ $movies->id }}"
                                                            @if ($movies->status == 1) {{ 'checked' }} @endif /></a>
                                                @endif
                                                @if ($movies->video_type!="GoogleDrive")
                                                <a href="{{ url('admin/movies/subtitle/' . $movies->id) }}"
                                                    class="btn btn-icon waves-effect waves-light btn-success m-r-5"
                                                    data-toggle="tooltip" title="{{ trans('Generate Subtitle') }}"><i
                                                        class="fa fa-file-text" @disabled(true)></i></a>
                                                @endif
  ...
  <!-- Play Button -->
  <a href="javascript:void(0);" class="play-video" data-video-url="https://www.youtube.com/watch?v=your_video_id">Play Video</a>
    ...
    $(document).ready(function() {
      ...
      $('.play-video').on('click', function() {
        ...
        if (videoUrl.includes('youtube.com')) {
          videoIframe.attr('src', videoUrl.replace('watch?v=', 'embed/') + '?autoplay=1');
        } else if (videoUrl.includes('vimeo.com')) {
          videoIframe.attr('src', videoUrl.replace('vimeo.com/', 'player.vimeo.com/video/') + '?autoplay=1');
        } else {
      ...
      $('#videoModal').on('hidden.bs.modal', function () {
        ...
        // Force stop video by setting a parameter that stops playback
        if (src.includes('youtube.com')) {
          videoIframe.attr('src', src.split('?')[0] + '?autoplay=0');
        } else if (src.includes('vimeo.com')) {
          videoIframe.attr('src', src.split('?')[0] + '?background=1');
        } else {
          videoIframe.attr('src', ''); // Clear iframe src for other URLs
        }
      });
    });
  </script>
