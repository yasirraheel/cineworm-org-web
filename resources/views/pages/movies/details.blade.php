...
@else
    @section('head_description', Str::limit(stripslashes($movies_info->video_description), 500))
@endif
                ...
                <div class="col-lg-7 col-md-12 col-sm-12 col-xs-12 mb-4">
                    <div class="detail-poster-area">
                        ...
                        <div class="play-icon-item">
                            <a class="icon"
                                href="{{ URL::to('movies/watch/' . $movies_info->video_slug . '/' . $movies_info->id) }}"
                                title="play">
                        ...
                        <div id="social-media" class="modal fade centered-modal" tabindex="-1" role="dialog"
                        ...
                        <div class="dtl-poster-img">
                            <img src="{{ URL::to('/' . $movies_info->video_image) }}"
                                alt="{{ stripslashes($movies_info->video_title) }}"
                                title="{{ stripslashes($movies_info->video_title) }}" />
                        </div>
                    </div>
                </div>
                <div class="col-lg-5 col-md-12 col-sm-12 col-xs-12 mb-4">
                    <div class="poster-dtl-item">
                        <h2><a href="{{ URL::to('movies/watch/' . $movies_info->video_slug . '/' . $movies_info->id) }}"
                                title="{{ stripslashes($movies_info->video_title) }}">{{ stripslashes($movies_info->video_title) }}</a>
                        </h2>
                        <ul class="dtl-list-link">
                            ...
                            @endforeach
                            <li>
                                <a href="{{ URL::to('movies?lang_id=' . $movies_info->movie_lang_id) }}"
                            ...
                            <div class="video-watch-share-item mb-3">
                                <div class="subscribe-btn-item" style="margin-left:0px !important">
                                    <a href="javascript:window['player2'].showLightbox();"
                                        title="{{ trans('words.watch_triler') }}"><i class="fa fa-play-circle"></i>
                        ...
                        @if (!is_null($movies_info->actor_id) > 0)
                            ...
                            <span class="des-bold-text"><strong>{{ trans('words.actors') }}:</strong>
                                <?php $a = '';
                                $n = count(explode(',', $movies_info->actor_id, 6)); ?>
                                @foreach (explode(',', $movies_info->actor_id, 6) as $i => $actor_ids)
                                    <a href="{{ URL::to('actors/' . App\ActorDirector::getActorDirectorInfo($actor_ids, 'ad_slug')) }}/{{ $actor_ids }}"
                                        title="actors">{{ App\ActorDirector::getActorDirectorInfo($actor_ids, 'ad_name') }}</a><?php if ($i + 1 != $n) {
                                            echo $a = ',';
                                        } ?>
                                @endforeach
                        ...
                        @if (!is_null($movies_info->director_id) > 0)
                            <span class="des-bold-text"><strong>{{ trans('words.directors') }}:</strong>
                                ...
                                <?php $a = '';
                                $n = count(explode(',', $movies_info->director_id, 6)); ?>
                                @foreach (explode(',', $movies_info->director_id, 6) as $i => $director_ids)
                                    <a href="{{ URL::to('directors/' . App\ActorDirector::getActorDirectorInfo($director_ids, 'ad_slug')) }}/{{ $director_ids }}"
                                        title="directors">{{ App\ActorDirector::getActorDirectorInfo($director_ids, 'ad_name') }}</a><?php if ($i + 1 != $n) {
                                            echo $a = ',';
                                        } ?>
                                @endforeach
